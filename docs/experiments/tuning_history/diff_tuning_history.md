...
## 9. Plan A/B — 조류 적응 3-arm과 속도 축 (2026-08-10, advanced_experiments_todo)

### 9.1 속도 축 (Plan B) — 새 단조 축 확보, diff 전 속도 선두

`speed_scale`(ScanCfg ramp 스케일, 상태머신·MPC 참조·평가 타깃 일관) × Stage3,
seeds 0–2 평균:

| 방법 | 1× (0.2 m/s) | 1.5× | 2× |
|---|--:|--:|--:|
| nominal | 969 | 1,535 | **2,149 (단조 2.2×)** |
| bo | 679 | 911 | 940 |
| ssi | 697 | 916 | 962 |
| **diff** | **681** | **909** | **910** |

**판독**: 속도 축은 "튜닝 vs 무튜닝"을 가르는 단조 축 (E2 유체 축의 평탄성과 대조).
튜닝 3종은 +34% 후 평탄 — 0.2 m/s 시대의 가중치가 2×에서도 유효. diff는 전 속도
선두/동률, 상태머신도 정상 (2×에서 정확히 4 사이클). E4c/논문 F3 대체 축 후보.

### 9.2 조류 fine-tune (Plan A) — 학습 불안정 발견·수정이 본체

**red-flag 사건**: lr 5e-4로 조류(0.25 m/s) fine-tune/from-scratch한 정책이
step25/s2에서 265k/906k 폭주 (zero-shot 3,812 대비). 진단:
- 학습 로그: at_ceiling 0.33–0.42, w_roll 5000 고정 — **지속 외란 하에서 추종 손실이
  가중치를 천장으로 밀고 기본 l_u(1e-3)의 반대 압력이 지는** 기록된 병리의 재발.
- 평가 aux: 하드시드에서 t=0부터 가중치 73% 바닥 고정(OOD 프리즈) → cycles 0.
- **부모 논문의 한계절("rapid weight updates may destabilize") 실증** — 그들의
  lr 2.9e-5가 우연이 아님.

**수정 과정의 함정 (기록)**: 1차 재학습(lr 3e-5)이 구 run과 **비트 단위 동일**하게
재생됨 — `--resume_ckpt`가 Adam state를 로드하며 param_groups의 lr(5e-4)까지 복원해
CLI `--lr`를 조용히 덮어씀. 수정: opt state 로드 후 param_groups lr을 CLI로 재설정.
(같은 seed의 결정론 재생 덕에 즉시 발각 — 재현성 인프라의 가치.)

**수정 결과**: ft25c(lr 3e-5, 클로버 수정판)가 폭주 셀에서 **3,804** (70× 개선,
zero-shot과 동률, 2.0 사이클) — 학습 중 at_ceiling 0.33은 평가 건강성의 대리지표가
아님도 확인. l_u 0.01 병행 변형(ftd)은 비교용으로 학습.

**건강 레시피(lr 3e-5) 기준 3-arm 최종 표** (seeds 0–2 평균, objective ↓):

| 조건 | zero-shot (V8a) | fine-tuned (10k) | from-scratch (60k) |
|---|--:|--:|--:|
| step 0.15 | **1,095** | 1,128 | 1,185 |
| sine 0.15 | 980 | **952** | 1,020 |
| step 0.25 | 1,355 | **1,350** | 2,056 |
| sine 0.25 | **1,050** | 1,054 | 1,230 |

**Plan A 판정**: 조류 축에서도 **zero-shot ≈ fine-tune** (±3% 내), from-scratch는
오히려 열세 (V8a의 공칭+자세 커리큘럼을 조류-단독 학습이 대체 못 함). E2b′(플랜트
축)와 합치면: **이 도메인에서 V8a zero-shot은 시험한 모든 적응 축(플랜트·조류)에서
environment-specific 상한에 이미 도달** — diff의 온라인 학습 가치는 배치-시 적응이
아니라 (a) 오프라인에서 상황-적응 정책을 만드는 도구, (b) 적응이 실제 필요한 경우를
위한 보험으로 서술한다. 단, **온라인 학습을 실기체에서 켜려면 lr 3e-5 안정화가
전제** (본 절의 불안정 실증 — 안전 관점의 실용 지침).

ftd(lr 3e-5 + l_u 0.01)는 학습만 완료·미평가 보류 — lr 단독으로 충분했고, l_u 축은
백로그의 보수-lr 대조와 함께 여력 시 부록감.
