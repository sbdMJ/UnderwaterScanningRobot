...
## 10. SSI-MPC 논문(T-RO, arXiv:2407.04143) 실험 설계 미러링

원 논문 PDF에서 확인한 설계를 우리 실험 구조에 1:1 대응시킨다 (2026-08-04 확정):

| SSI-MPC 논문 | 원 논문 수치 | 우리 대응 |
|---|---|---|
| §V-B 본 비교: 오차 시계열 + 샘플 궤적 + **예측오차 감소 곡선** + 계산시간 표(mean±SD) | cart-pole 6 s × **10회** 랜덤 초기화; Nominal/NS-MPC/GP-MPC 비교 | E1 (40 trial/방법) + Fig F1·F2·F5 + `controller_cost` 표 |
| §V-C 민감도: M·η 그리드 스윕, 오차 시계열(변수별) + 누적오차 vs η 곡선 + 계산시간 vs M 표 | M∈{50,125,250,500,1000}, η∈{0.001,0.01,0.05,0.1,0.2,0.4,0.6,0.8,1} | `e4_sensitivity_ssi.yaml` (ssi_n_rf·ssi_lr을 조건으로 스윕 — 러너가 오늘 실행 가능) + 제안 방법 민감도는 werr_ub/history_len 변형 재학습 후 동일 형식 |
| §V-D 외란 스윕: 궤적 4종 × 최대속도 스윕, **조건당 5회**, 추종오차 vs 속도 곡선 | vm 스윕이 공력 외란 강도를 변조 | E2 (성능 vs DR 강도) + E3 (조류) — 조건당 seed 수를 논문과 맞춰 **5**로 상향 가능(기본 3, 여력 시) |
| §VI 하드웨어: 하이퍼파라미터 축소·고정 | 시뮬 M=50/η=0.25 → 하드웨어 **M=25/η=0.05 고정**, Jetson Orin NX 온보드 | E5에서 SSI 하이퍼파라미터 축소·고정 관행 준용, Jetson 벤치(E4c)와 연결 |

원 논문이 검증해주는 우리 구현 선택 두 가지: (a) 쿼드로터 feature = 속도·쿼터니언·각속도·
정규화 추력 (**절대 위치 제외**) — `ssi_controller.INPUT_MASK_DEFAULT`와 동일 구성.
(b) ω 표준편차는 feature 크기에 맞춰 작게(그들은 비정규화 feature에 0.01) — kernel_std를
§6 튜닝 대상으로 둔 근거.
