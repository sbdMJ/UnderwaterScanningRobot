#!/usr/bin/env bash
# e5_hwdrag parallel runner: 4 workers = (method x condition). Each CELL is its
...
# runner is resumable. Host-side logs: isaaclab/logs/e5_hwdrag_run/.
set -uo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
CFG=../marinelab/scripts/experiments/configs/e5_hwdrag.yaml
LOGDIR="$REPO/isaaclab/logs/e5_hwdrag_run"
RESULTS="$REPO/experimental_results/e5_hwdrag/metrics"
SEEDS=(0 1 2 3 4)
mkdir -p "$LOGDIR"
...
worker() {  # $1 = worker index, $2 = method, $3 = condition
  ...
  for s in "${SEEDS[@]}"; do
    ...
    if UWS_LOGS_MOUNT="$HOME/docker/isaac-sim/e5hd_w$i" "$REPO/docker/run.sh" \
...
for method in nominal ssi; do
  for cond in hwdrag hwdrag_trim; do
