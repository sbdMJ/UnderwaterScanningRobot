...
# ---------------------------------------------------------------------------
# What d_ref is measured to (settled 2026-08-03)
# ---------------------------------------------------------------------------


def test_sonar_convention_puts_the_beam_reading_at_d_ref():
    """The MPC's radial target must make the SONAR read d_ref, not the hull clear it.

    Cross-checked against the estimator's mounted range model rather than re-deriving the
    geometry here, so the controller target and the filter's measurement model cannot drift
    apart.
    """
    from marinelab.tasks.pkrc_wallscan.wall_frame_ekf import mounted_sonar_range

    cfg = mr.WallScanMPCCfg()
    assert cfg.d_ref_is_sonar_range is True
    assert cfg.r_des == pytest.approx(6.0 - 1.5 - 0.10)
    reading = mounted_sonar_range(cfg.r_des, 0.0, cfg.tank_radius,
                                  (cfg.sonar_mount_x, 0.0), 0.0)
    assert reading == pytest.approx(cfg.d_ref, abs=1e-9)


def test_hull_convention_is_still_available_and_differs_by_the_mount():
    cfg = mr.WallScanMPCCfg(d_ref_is_sonar_range=False)
    assert cfg.r_des == pytest.approx(6.0 - 1.5)
    assert mr.WallScanMPCCfg().r_des == pytest.approx(cfg.r_des - 0.10)


def test_the_two_conventions_bracket_the_hull_clearance():
    """Sonar convention keeps MORE hull margin, which is the safe direction to be wrong in."""
    sonar_cfg, hull_cfg = mr.WallScanMPCCfg(), mr.WallScanMPCCfg(d_ref_is_sonar_range=False)
    hull_margin = lambda c: c.tank_radius - c.r_des  # noqa: E731
    assert hull_margin(sonar_cfg) == pytest.approx(1.6)
    assert hull_margin(hull_cfg) == pytest.approx(1.5)
    assert hull_margin(sonar_cfg) > sonar_cfg.d_ref


def test_a_zero_mount_makes_the_conventions_identical():
    a = mr.WallScanMPCCfg(sonar_mount_x=0.0)
    b = mr.WallScanMPCCfg(sonar_mount_x=0.0, d_ref_is_sonar_range=False)
    assert a.r_des == pytest.approx(b.r_des)
