...
def results(tmp_path):
    for seed, cycles in enumerate([2.0, 1.8, 2.2]):
        _write_metrics(tmp_path, "nominal", "nominal", seed, cycles, 1.5)
...
def test_collect_parses_tags(results):
    ...
    assert len(rows) == 4
    fixed = [r for r in rows if r["method"] == "nominal"]
...
def test_write_table_csv(results, tmp_path):
    ...
    row = next(r for r in table if r["method"] == "nominal" and r["metric"] == "cycles_mean")
...
def test_write_table_tex(results, tmp_path):
    ...
    assert "Nominal NMPC & 2 $\\pm$ 0.2" in tex          # mean ± sd, method label mapped
    assert "Diff-WMPC (ours)" in tex
    assert tex.index("Nominal NMPC") < tex.index("Diff-WMPC")  # METHOD_ORDER respected
...
def test_write_table_tex_inf_renders_dash(tmp_path):
    ...
    (tmp_path / "metrics_nominal_nominal_s0.json").write_text(json.dumps(payload))
