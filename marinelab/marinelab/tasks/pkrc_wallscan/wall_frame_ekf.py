...
__all__ = ["WallFrameEKF", "WallFrameEKFCfg", "sonar_range", "sonar_jacobian",
           "mounted_sonar_range", "mounted_sonar_jacobian",
...
def mounted_sonar_range(r: float, phi: float, tank_radius: float,
                        mount_xy=(0.0, 0.0), yaw_offset: float = 0.0) -> float:
    ...
    return sonar_range(r_sen, phi_sen, tank_radius)


def mounted_sonar_jacobian(r: float, phi: float, tank_radius: float,
                           mount_xy=(0.0, 0.0), yaw_offset: float = 0.0) -> tuple[float, float]:
    ...
    dt_drs, dt_dps = sonar_jacobian(r_sen, phi_sen, tank_radius)
...
class WallFrameEKF:
    ...
    def update_sonar(self, measured_range: float) -> bool:
        ...
        pred = mounted_sonar_range(r, phi, self.cfg.tank_radius, mount, yaw_off)
        dt_dr, dt_dphi = mounted_sonar_jacobian(r, phi, self.cfg.tank_radius, mount, yaw_off)
