...
* ``r_des`` from ``d_ref`` — see :class:`WallScanMPCCfg` for the convention, which is not
  self-evident and cost 10 cm of apparent error once already,
...
#   [r_des, z_ref, s_ref, v_tan_des, v_z_des, theta_anchor, s_anchor, d_force_world(3), d_moment_body(3)]
# theta_anchor/s_anchor carry the env's *unwrapped* arc-length bookkeeping into the solve
# (see arc_length below); the trailing 6 are the residual wrench from
# ``wrench_observer.WrenchObserver`` -- zero reproduces the pre-observer controller exactly.
#
# The MOMENT half was added 2026-08-04. The original 3-entry force-only slot could not address
# the failure it was reserved for: under stress DR the standing tilt is 13.83 deg (measured,
# ``results/metrics_dwDRds_*.json``) and a force resolved at the body origin produces no moment
# at all, so no value of a force-only disturbance can trim it. The two frames differ on purpose
# -- see the wrench_observer docstring: excess buoyancy is constant in the WORLD frame, a
# CoB/CoG trim moment is constant in the BODY frame.
NP_REF = 7
ND_FORCE = 3
ND_MOMENT = 3
ND = ND_FORCE + ND_MOMENT
...
class WallScanMPCCfg:
    ...
    step_dt: float = 0.02
    dt_mpc: float = 0.05
    # What ``d_ref`` is measured TO. The task config does not say (``d_ref: float = 1.5
    # # target wall distance, m``), and the env itself uses both conventions: the tracking
    # reward compares ``d_ref`` against the SONAR reading (``wallscan_env.py:331``, on
    # ``gt["wall_dist"]`` from the nominal mount) while the collision terminal compares
    # ``d_min`` against ``radial_clearance``, which is the HULL.
    #
    # 2026-08-03: confirmed with the user that ``d_ref`` is the SONAR standoff. That is the
    # quantity the survey cares about — the beam footprint scales with it, so holding it
    # constant is what makes coverage uniform — while hull clearance is handled separately as
    # a safety floor by ``d_min``.
    #
    # This module previously used ``r_des = tank_radius - d_ref`` (the hull reading), which
    # aimed the MPC 10 cm inside the RL policy's target and made every
    # ``eval_metrics.wall_dist_err_cm`` comparison look 10 cm worse than it was. Measured:
    # station-keeping held ``|pos_xy|`` at 4.5066 against a 4.5000 target (0.7 cm), yet the
    # metric reported 10.66 cm because it compares the mounted sonar range (1.3934) to 1.5.
    d_ref_is_sonar_range: bool = True
    sonar_mount_x: float = 0.10  # = SensorCfg.sonar_mount_pos[0]; forward offset of the beam

    @property
    def r_des(self) -> float:
        """Target distance from the tank AXIS (neither the hull clearance nor the sonar range).

        Under the sonar convention the beam starts ``sonar_mount_x`` further out, so making it
        read ``d_ref`` means holding the vehicle that much closer in::

            sonar reading = tank_radius - (r + mount_x) = d_ref   ->   r = R - d_ref - mount_x

        Exact at zero beam offset and linear in the mount, which is enough here: the closed
        loop settles at 0.19-0.32 deg of heading error, where the second-order range effect of
        the offset is sub-micrometre (see :func:`clearance_bias`). Putting the full mounted
        sonar model into the MPC cost would be exact at any offset but couples the radial and
        heading terms for no measurable gain.
        """
        offset = self.sonar_mount_x if self.d_ref_is_sonar_range else 0.0
        return self.tank_radius - self.d_ref - offset
