...
from .mpc_reference import ND, ND_FORCE, NE, NP_REF
...
def _continuous_dynamics(x, u, prm: PlantParams, B, d_force_world, d_moment_body):
    ...
    # Residual wrench from the observer, held constant over the horizon (the offset-free NMPC
    # assumption). Force arrives in the WORLD frame and is rotated in; the moment is already
    # body-frame. See mpc_reference's ND comment for why the frames differ.
    f_dist = R.T @ d_force_world

    tau = ca.DM(np.asarray(B, dtype=float)) @ u
    f_tot = tau[0:3] + hydro[0:3] + f_buoy + f_grav + f_dist
    m_tot = tau[3:6] + hydro[3:6] + m_buoy + d_moment_body
...
def build_ocp(prm: PlantParams, cfg: mref.WallScanMPCCfg, *, N: int, sensitivity: bool,
    ...
    model.p = p
    d_force_world = p[NP_REF:NP_REF + ND_FORCE]
    d_moment_body = p[NP_REF + ND_FORCE:NP_REF + ND]

    def f(xx, uu):
        return _continuous_dynamics(xx, uu, prm, B, d_force_world, d_moment_body)
...
class WallScanMPC:
    ...
    def param_matrix(self, ref: dict, theta_anchor: float, s_anchor: float,
        ...
        ``d_wrench`` is ``[force_world(3), moment_body(3)]`` from
        :class:`~marinelab.tasks.pkrc_wallscan.wrench_observer.WrenchObserver`, written to EVERY
        stage: the observer's estimate is a constant over the horizon, which is the offset-free
        NMPC assumption and the reason it removes a standing offset rather than merely reacting to
        one. ``None`` writes zeros and reproduces the pre-observer controller bit for bit.
        """
        d = np.zeros(ND) if d_wrench is None else np.asarray(d_wrench, float).reshape(ND)
