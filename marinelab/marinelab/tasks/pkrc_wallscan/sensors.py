...
class SensorCfg:
    ...
    dvl_bias_dr: float = 0.0       # m/s     (DVL velocity offset, ADDITIVE)
    # DVL scale-factor error (2026-08-03): per-episode multiplicative error, drawn uniform in
    # +-<field> as a FRACTION. The DVL-A50 datasheet specifies "Long term accuracy +-1.01%
    # (Standard) / +-0.1% (Performance)" -- a percentage, i.e. proportional to velocity, not a
    # constant offset. The two error forms are not interchangeable for this task:
    #
    #   additive 0.01 m/s  -> s drift = 0.01 * 180 s          = 1.8 m   (accrues while parked)
    #   scale    1.01%     -> s drift = 1.01% * 4 m of sway   = 4 cm    (accrues with distance)
    #
    # The sway legs only travel ~4 m per 180 s episode (2 cycles x 2 legs x 1 m) and v_y is ~0
    # through the vertical legs, so the additive model is ~45x pessimistic here. 0 = off, which
    # keeps the legacy additive-only behaviour.
    dvl_scale_dr: float = 0.0      # fraction (e.g. 0.0101 for +-1.01%)
    # DVL update rate [Hz]; 0 = every control step (legacy). The A50 pings at 4-15 Hz against a
    # 50 Hz control loop, so a real reading is HELD for 3-12 steps. Drawing fresh noise every
    # step instead lets the filter average it away far faster than reality allows -- optimistic.
    dvl_rate_hz: float = 0.0
    # Sonar beam half-angle [rad]; 0 = single ray (legacy). Ping1D is 25 deg full width.
...
class SensorCfgDatasheet(SensorCfg):
    ...
    * The 25 deg beam is WIDE — a 0.66 m footprint at 1.5 m. Every sonar model in this repo
    ...
    # --- DVL-A50 (Water Linked, doc WL-21035-4-v4) ----------------------------------------
    # "Long term accuracy +-1.01% (Standard version)". The Performance version is +-0.1%; pick
    # Standard until the actual unit is confirmed, since it is the conservative reading.
    dvl_scale_dr: float = 0.0101
    # Zeroed on purpose: the datasheet expresses DVL error as a percentage, so there is no
    # additive-offset figure to put here. Keeping the placeholder 0.01 m/s alongside the scale
    # error would double-count and reinstate the 1.8 m drift the scale form removes.
    dvl_bias_dr: float = 0.0
    # 15 Hz: the top of the published "4 - 15 Hz (adaptive to altitude)" band. At the 1-8.5 m
    # altitudes this tank allows, two-way travel time is ~1-11 ms, so the rate is limited by
    # processing rather than range and sits near the top. 4 Hz is the worst case; sweep it if the
    # real unit reports slower.
    dvl_rate_hz: float = 15.0
    # NOT datasheet-backed: "Velocity resolution 0.1 mm/s" is quantisation, not a noise std, and
    # no std is published. Left at the placeholder.
    # --- Ping1D beam ----------------------------------------------------------------------
    # 25 deg full width (-3 dB) -> 12.5 deg half-angle. Consequences run BOTH ways, see
    # geometry.sonar_wall_distance: the reading becomes immune to crab below 12.5 deg (the
    # one-sided over-read at 30 deg falls from 16.5 cm to 5.4 cm) and simultaneously carries
    # ZERO heading information inside the cone.
    sonar_beam_half_angle: float = 0.2182
    # --- 3DM-GV7 ------------------------------------------------------------------------
...
def dvl_hold_steps(cfg: SensorCfg, step_dt: float) -> int:
    """How many control steps one DVL reading is held for. 1 = fresh every step (legacy)."""
    if cfg.dvl_rate_hz <= 0.0:
        return 1
    return max(1, int(round(1.0 / (cfg.dvl_rate_hz * step_dt))))
...
def apply_sensors(gt: dict, cfg: SensorCfg, gen: torch.Generator | None, bias: dict | None = None) -> dict:
    ...
    One entry is multiplicative rather than additive: ``lin_vel_scale`` is the DVL's
    scale-factor error, applied as ``v * (1 + scale)`` before noise and any additive offset.
    That is the form the DVL-A50 datasheet specifies (accuracy in percent); see
    ``SensorCfg.dvl_scale_dr``.
    """
    ...
    # Standard sensor chain: scale error, then noise, then any additive offset.
    lin_vel_out = (lin_vel * (1.0 + b.get("lin_vel_scale", 0.0))
                   + noise(n, 3, std=cfg.dvl_noise) + b.get("lin_vel", 0.0))
